...
class S16 {
    ...
    fun test() = runTest(testDispatcher) {
        ...
        composeTestRule.onAllNodes(hasTestTag("LazyColumn")).assertCountEquals(1)

        composeTestRule.waitUntil("ColumnまたはRowが表示されている", 5000) {
            val columnOrRowCount =
                composeTestRule.onAllNodes(hasTestTag("Column") or hasTestTag("Row"))
                    .fetchSemanticsNodes().size
            columnOrRowCount >= 4
        }
        val textSize = composeTestRule.onAllNodes(hasTestTag("Text")).fetchSemanticsNodes()
            .size
        assertTrue(textSize >= 7, "ColumnまたはRowの中にTextがありません")
    }
}
